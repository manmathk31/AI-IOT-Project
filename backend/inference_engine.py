...
def run_inference(data_queue, session_factory):
    ...
    while True:
        try:
            ...
            if failed_sensors:
                print(f"[Inference] ⚠ Sensors failed ({FAILURE_THRESHOLD}+ missing): {failed_sensors}")
                ...
                for field in SENSOR_FIELDS:
                    if consecutive_missing[field] == FAILURE_THRESHOLD:
                        ...
                        if not existing:
                            ...
                            print(f"[Inference] ⚠ Sensor failure alert created for '{field}'")

                # 6. Check for flame override
                # GOLDEN RULE: only trigger flame when sensor EXPLICITLY sends flame=1
                if reading["flame"] == 1 and consecutive_missing.get("flame", 0) == 0:
